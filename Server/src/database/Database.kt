            ...
            if (result > 0) {
                ...
                if (rowsAffected == 0) {
                    ...
                    if (!query.contains("DELETE FROM") && !query.contains("INSERT INTO")) {
                        ...
                        query = if (query.contains("', parameters"))
                            query.substring(query.indexOf("UPDATE"), query.indexOf("', parameters"))
                        else
                            query.substring(query.indexOf("UPDATE"))
                        ...
                        val rows: String = if (newQuery.toString().contains("WHERE"))
                            newQuery.substring(newQuery.indexOf("SET ") + "SET ".length, newQuery.indexOf("WHERE "))
                        else
                            newQuery.substring(newQuery.indexOf("SET ") + "SET ".length)

                        // Construct an array of every front row
                        val frontRows = rows.replace(" = ?, ", ", ").replace(" = ? ", ", ").split(", ").toTypedArray()
